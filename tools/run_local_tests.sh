...
echo "+ Starting mariadb:latest DB server container..."
...
echo "+ Stopping mariadb:latest DB server container..."
docker stop kbs-db
...
echo "+ Starting postgres:latest DB server container..."
docker run --detach --rm \
           -p 5432:5432 \
...
echo "+ Running: cargo test for postgres using postgres:latest"
cargo test
...
export KBS_DB=sev_attest_test.db
